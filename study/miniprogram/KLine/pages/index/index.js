...
Page({
    ...
    drawSearchRes(d, p, index) {
        ...
        this.data.showDataStart = this.data.allData.length - index - Math.floor(globalData.bn / 2);
    ...
    draw() {
        ...
        if (this.data.searchResFlag) {
            ...
            let startMillTime = new Date(...this.data.showData[this.data.showData.length - globalData.bn].date.replace(/^\D+|\D+$/g, '').split(/\D/)).getTime();
                ...
                this.data.showData.forEach((item, showDataIndex) => {
                    if (item && item.date) {
                        ...
                        if (itemDateMillTime == searchDateMillTime) {
                            const index = showDataIndex - (this.data.showData.length - globalData.bn);
                            const searchObj = { showSearchRes: true };
                            if (!(index < 0 || index >= globalData.bn || item.invalide)) {
                                this.setData({
                                    showSearchRes: true,
                                    searchResXaxis: Math.round(globalData.mw + globalData.bw * (index + 1) - 0.5) + 'px',
