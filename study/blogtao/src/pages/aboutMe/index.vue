  ...
  Effect3D.prototype = {
    ...
    init() {
      ...
      Aui.each(self.path, (i) => {
        ...
        const domStrFunc = wrap => {
          return `<div class="${wrap}" style="background:${allElem[i].bg};">
                    <div class="title">${allElem[i].content}</div>
                    <div class="cont">${allElem[i].content}</div>
                  </div>`;
        };
  ...
  mounted() {
    Aui.ready(() => {
      if (/ie/ig.test(Aui.browser())) {
        ...
        Effect(allElem.map(() => ''), 200, 300, Aui.byID('#aboutme'), 2000, -10, 0).init();
      }
    });
  },
  methods: {
